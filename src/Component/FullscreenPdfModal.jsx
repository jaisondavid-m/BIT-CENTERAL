...
const ZOOM_MIN = 0.5;   // raised from 0.3 — helps avoid huge canvas on mobile
const ZOOM_MAX = 4.0;
// Safari iOS hard-limits canvas area to ~16 MP
...
/** Returns a safe scale that keeps canvas within MAX_CANVAS_AREA */
function safeScale(naturalW, naturalH, desiredScale, dpr) {
const w = naturalW * desiredScale * dpr;
const h = naturalH * desiredScale * dpr;
const area = w * h;
if (area <= MAX_CANVAS_AREA) return desiredScale * dpr;
return desiredScale * dpr * Math.sqrt(MAX_CANVAS_AREA / area);
}
...
const firstPageHRef = useRef(0);
const searchInputRef = useRef(null);
// Tracks the zoom value that is currently rendered (or being rendered)
const renderedZoomRef = useRef(null);
const renderGenRef = useRef(0); // generation counter to kill stale renders
...
}, [showSearch, onClose]); // eslint-disable-line react-hooks/exhaustive-deps
...
(async () => {
  try {
    ...
    if (isGdrive) {
      if (allowExternalActions) { setLoadState("blocked"); return; }
...
(async () => {
  try {
    ...
    // Measure all pages to find the widest
    let maxW = 0, maxH = 0;
    for (let i = 1; i <= doc.numPages; i++) {
      const page = await doc.getPage(i);
      const vp = page.getViewport({ scale: 1 });
      if (vp.width > maxW) { maxW = vp.width; maxH = vp.height; }
      page.cleanup();
    }
    firstPageWRef.current = maxW;
    firstPageHRef.current = maxH;
...
}, [pdfSource]); // eslint-disable-line react-hooks/exhaustive-deps
...
```
// Skip if zoom hasn't actually changed (prevents re-render on fit mode resize jitter)
...
(async () => {
  // Wait two frames so React can commit canvas DOM nodes
  ...
  // Cap DPR to 2 — Safari/iOS has a canvas area limit and DPR=3 blows it up
  const dpr = Math.min(window.devicePixelRatio || 1, 2);

  try {
    for (let pageNumber = 1; pageNumber <= doc.numPages; pageNumber++) {
      if (renderGenRef.current !== gen) return; // stale — abort

      const canvas = canvasRefs.current[pageNumber - 1];
      if (!canvas) continue;

      const page = await doc.getPage(pageNumber);
      if (renderGenRef.current !== gen) { page.cleanup(); return; }

      const naturalVp = page.getViewport({ scale: 1 });
      const scale = safeScale(naturalVp.width, naturalVp.height, zoom, dpr);
      ...
      // Safari sometimes returns null context if canvas is too large — guard it
      ...
      try {
        ...
        // RenderingCancelledException is expected when zoom changes mid-render
    ...
    if (renderGenRef.current === gen) setLoadState("ready");
  } catch (error) {
    if (renderGenRef.current !== gen) return;
...
return () => {
  // Bump generation so any running async loop exits
  renderGenRef.current += 1;
  renderTasksRef.current.forEach((task) => { try { task.cancel(); } catch {} });
};
```
...
const page = await doc.getPage(i);
const naturalVp = page.getViewport({ scale: 1 });
// Use a very small scale for thumbnails — safe on all devices
const thumbScale = safeScale(naturalVp.width, naturalVp.height, 0.18, dpr);
...
// Debounced so rapid zoom clicks don’t fire dozens of render passes
...
const changeZoom = (delta) => handleZoomDelta(delta);
...
```
    <div style={S.fileName}>
      <span style={S.fileNameText} title={name}>{name}</span>
    ...
    <div style={S.row}>
      ...
      {canShowNav && (
        <>
          ...
          <button
            ...
            style={S.iconBtn(showSearch ? "active" : "default")}
            title="Search"
          >
            <Search size={15} />
          </button>
      ...
      {hideExternalActions && !showOpenNew && !showDownload && (
        <div
          aria-hidden="true"
          style={{ width: 32, height: 32, borderRadius: 6, background: "#000", border: "1px solid #111", flexShrink: 0 }}
        />
      )}
    </div>
  </div>
  ...
  <div style={S.body}>
    ...
    <div ref={scrollRef} style={S.canvasArea}>
      {loadState === "fetching" && (
        <StatusBox><Spin /> Loading PDF…</StatusBox>
      )}
      {loadState === "error" && (
        <StatusBox danger><span style={{ fontSize: 20 }}>⚠</span>{errorMsg}</StatusBox>
      )}
      ...
      {loadState === "blocked" && embedUrl && (
        <div style={{ display: "flex", flexDirection: "column", width: "100%", flex: 1, minHeight: 0 }}>
          ...
          <iframe
            src={`${embedUrl}${embedUrl.includes("?") ? "&" : "?"}rm=minimal`}
            style={S.gdriveFrame}
            title="PDF Preview"
            allow="autoplay"
          />
      ...
      {loadState === "rendering" && numPages > 0 && (
        <StatusBox><Spin /> Rendering…</StatusBox>
      )}
    </div>
  </div>
...
toolbar: {
display: “flex”, alignItems: “center”, gap: 6, padding: “0 10px”, height: 48,
flexShrink: 0, background: “#1a1a1a”, borderBottom: “1px solid #272727”,
userSelect: “none”, overflowX: “auto”, overflowY: “hidden”,
},
...
akBadge: {
flexShrink: 0, fontSize: 10, fontWeight: 700, background: “#7c3aed22”, color: “#a78bfa”,
border: “1px solid #7c3aed55”, borderRadius: 4, padding: “1px 7px”, letterSpacing: “0.04em”,
},
iconBtn: (variant) => ({
display: “flex”, alignItems: “center”, justifyContent: “center”,
width: 30, height: 30, borderRadius: 6, border: “1px solid”, cursor: “pointer”,
transition: “all .1s”, flexShrink: 0,
background: variant === “active” ? “#1e3a7a” : variant === “danger” ? “#3a0f0f” : “transparent”,
borderColor: variant === “active” ? “#3b6ef8” : variant === “danger” ? “#7a1e1e” : “#2e2e2e”,
color: variant === “active” ? “#7aaeff” : variant === “danger” ? “#f87171” : “#ccc”,
}),
pageBox: { display: “flex”, alignItems: “center”, gap: 4, fontSize: 12, color: “#bbb” },
pageInput: {
width: 38, background: “#222”, border: “1px solid #333”, borderRadius: 4,
color: “#e0e0e0”, fontSize: 12, textAlign: “center”, padding: “2px 4px”, outline: “none”,
},
pageTotal: { color: “#555” },
zoomBadge: {
background: “#222”, border: “1px solid #333”, borderRadius: 5, color: “#ccc”,
fontSize: 11, fontWeight: 600, padding: “3px 9px”, cursor: “pointer”,
minWidth: 46, textAlign: “center”, flexShrink: 0,
},
downloadBtn: {
display: “flex”, alignItems: “center”, gap: 5, flexShrink: 0,
background: “#052e16”, border: “1px solid #16a34a55”, borderRadius: 6,
color: “#4ade80”, fontSize: 12, fontWeight: 700, padding: “4px 11px”, textDecoration: “none”,
},
mobileNav: {
display: “flex”, alignItems: “center”, justifyContent: “space-between”,
padding: “4px 12px”, background: “#161616”, borderBottom: “1px solid #222”, flexShrink: 0,
},
mobileNavBtn: {
display: “flex”, alignItems: “center”, justifyContent: “center”,
width: 32, height: 32, borderRadius: 6, border: “1px solid #2e2e2e”,
background: “transparent”, color: “#ccc”, cursor: “pointer”,
},
mobileNavLabel: { fontSize: 12, fontWeight: 500, color: “#999” },
searchBar: {
display: “flex”, alignItems: “center”, gap: 8, padding: “6px 10px”,
background: “#161616”, borderBottom: “1px solid #222”, flexShrink: 0,
},
searchInput: {
flex: 1, maxWidth: 320, background: “#222”, border: “1px solid #333”,
borderRadius: 4, color: “#e0e0e0”, fontSize: 13, padding: “5px 10px”, outline: “none”,
},
body: { flex: 1, display: “flex”, overflow: “hidden” },
sidebar: {
width: 140, background: “#151515”, borderRight: “1px solid #222”,
overflowY: “auto”, flexShrink: 0, padding: “8px 6px”,
display: “flex”, flexDirection: “column”, gap: 6,
},
thumbBtn: {
background: “transparent”, border: “2px solid transparent”, borderRadius: 5,
cursor: “pointer”, padding: 4, display: “flex”, flexDirection: “column”,
alignItems: “center”, gap: 4, width: “100%”, transition: “border-color .15s, background .15s”,
},
thumbPlaceholder: { width: “100%”, aspectRatio: “0.77”, background: “#222”, borderRadius: 2 },
canvasArea: {
flex: 1, overflowY: “auto”, overflowX: “auto”, padding: “16px 12px 40px”,
display: “flex”, flexDirection: “column”, alignItems: “center”, gap: 12, background: “#1c1c1c”,
},
pageWrap: {
position: “relative”, background: “#fff”, borderRadius: 3, flexShrink: 0,
boxShadow: “0 4px 24px rgba(0,0,0,.55)”, overflow: “visible”,
width: “fit-content”, maxWidth: “none”,
},
pageLabel: {
position: “absolute”, top: 6, right: 8, background: “rgba(0,0,0,.5)”, color: “#fff”,
fontSize: 10, fontWeight: 600, borderRadius: 3, padding: “1px 5px”,
zIndex: 2, pointerEvents: “none”, userSelect: “none”,
},
gdriveNotice: {
display: “flex”, alignItems: “center”, gap: 8, flexShrink: 0,
background: “#1a2a1a”, border: “1px solid #1e4a1e”, borderRadius: “8px 8px 0 0”,
padding: “8px 14px”, fontSize: 12, color: “#86efac”,
},
gdriveFrame: { flex: 1, width: “100%”, border: “none”, minHeight: “calc(100vh - 160px)”, background: “#fff” },
statusBar: {
height: 26, background: “#111”, borderTop: “1px solid #1e1e1e”,
display: “flex”, alignItems: “center”, padding: “0 12px”, gap: 16,
fontSize: 11, color: “#555”, flexShrink: 0, userSelect: “none”,
},
};
